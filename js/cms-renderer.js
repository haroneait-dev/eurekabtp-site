...
export async function renderPage(slug, mountId = 'cms-content', options = {}) {
  ...
  // Après le rendu CMS, le layout a changé donc les anciens ScrollTrigger
  // pointent vers des positions périmées. On les détruit tous et on recrée
  // des animations fraîches sur tous les [data-animate] (statique + CMS).
  requestAnimationFrame(() => {
    ...
    // Reset visuel des éléments avant de recréer leurs animations
    document.querySelectorAll('[data-animate]').forEach(el => {
      window.gsap.set(el, { clearProps: 'all' });
    });

    // Recréer animations + ScrollTriggers avec les bonnes positions
    document.querySelectorAll('[data-animate]').forEach(el => {
      window.gsap.from(el, {
