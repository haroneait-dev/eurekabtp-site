import{s as m}from"./supabase-client-BPmHix10.js";function r(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function l(t){return t?r(String(t).replace(/ /g,"%20")):""}function f(t){return t?String(t).split(/\n\s*\n/).map(e=>`<p>${r(e.trim()).replace(/\n/g,"<br>")}</p>`).join(""):""}function b(t){const e=t.data||{};return`
    ...
    </section>`}const v={hero:b,split:$,process_intro:h,cta:y};async function k(t,e="cms-content",i={}){const n=document.getElementById(e);if(!n)return;const o=new Set(i.skipTypes||[]),{data:s,error:c}=await m.from("pages").select("id").eq("slug",t).maybeSingle();if(c||!s){console.warn("CMS: page not found for slug",t);return}const{data:u,error:d}=await m.from("sections").select("*").eq("page_id",s.id).eq("published",!0).order("order_index");if(d){console.error("CMS: error loading sections",d);return}const g=(u||[]).filter(a=>!o.has(a.type));g.length!==0&&(n.innerHTML=g.map(a=>{const p=v[a.type];return p?p(a):""}).join(""),window.gsap&&window.ScrollTrigger&&(document.querySelectorAll("[data-animate]:not([data-animated])").forEach(a=>{a.setAttribute("data-animated","true"),window.gsap.from(a,{y:30,opacity:0,duration:.75,ease:"power2.out",scrollTrigger:{trigger:a,start:"top 85%"}})}),window.ScrollTrigger.refresh&&window.ScrollTrigger.refresh()))}export{k as r};
